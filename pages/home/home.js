...
onShareAppMessage() {
  ...
  async onShow() {
    await this.checkBoosterStatus();
    this.getTabBar().init();
  },

  async checkBoosterStatus() {
      const userInfo = await getUser(true);
      if (userInfo && Number(userInfo.role) === 2) {
          this.setData({ isBooster: true });
      } else {
          this.setData({ isBooster: false });
      }
  },

  navToOrderSquare() {
      wx.navigateTo({ url: '/pages/order/square/index' });
  },
