...
      (await getCloudImageTempUrl(images)).forEach((image, index) => (orderItems[index].sku.image = image));
    ...
    this.setData({ order, addressEditable: order.status === ORDER_STATUS.TO_PAY });
  ...
  async onEditAddressTap() {
    ...
    try {
      ...
      try {
        await updateOrderDeliveryInfo({ orderId: this.data.order._id, accountId: deliveryInfo._id });
        this.setData({
          'order.game_account._id': deliveryInfo._id,
          'order.game_account.gameName': deliveryInfo.gameName,
          'order.game_account.gameId': deliveryInfo.gameId,
          'order.game_account.gamePlatform': deliveryInfo.gamePlatform,
        });
      } catch (e) {
        console.error(e);
        this.toast('更新游戏账号失败');
  ...
  onTrackingNumCopy() {
    wx.setClipboardData({
      data: this.data.order.tracking_number,
    });
  },
