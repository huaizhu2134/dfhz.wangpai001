...
Page({
  ...
  async onSubmitBtnClick() {
    const { goodRateValue, isAllowedSubmit, orderItemId, sku, uid } = this.data;
    const { textAreaValue } = this;

    if (!isAllowedSubmit) return;

    try {
      await createComment({ orderItemId, content: textAreaValue, rating: goodRateValue, spuId: sku.spu._id, userId: uid });
      Toast({
        context: this,
        selector: '#t-toast',
        message: '评价提交成功',
